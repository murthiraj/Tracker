...
function vehicleInfoInitialize() {
  ...
  Tracker.user.getVehicle(pageVehicleId, function(vehicleObject) {
  ...
  $('#change-vehicle-image-button').click(function(){
    Tracker.image_upload.newPicture( Tracker.vehicles.onPicSuccess);
  });
  ...
  if (vehicleStolen) {
    ...
    $('#remove').click(function() {
      ...
      if (deleteVehicle) {
        ...
        Tracker.user.removeVehicle(pageVehicleId, successFun, errorFun);
...
function vehicles() {
  this.onPicSuccess = function(base64, imageURI){
    ...
    file.save().then(function() {
      ...
      if (Tracker.currentPage == "new-vehicle") {
        // We cannot upload the file yet since the vehicle doesn't exist
        newVehicle.curImage = file;
        $('#vehicle-image').attr('src',imageURI); 
      } else {
        Tracker.user.updateVehicle(pageVehicleId, field, successFun, errorFun);
      }
